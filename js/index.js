...
function validate() {
  ...
  xhttp.open(
    "GET",
    "php/validateCredentials.php?action=is_admin&uname=" +
      uname +
      "&pswd=" +
      pswd,
    true
...
function validateAndSetup() {
  ...
  if (!validateName(pharmacy_name.value, "pharmacy_name_error"))
    pharmacy_name.focus();
  else if (!validateAddress(address.value, "address_error")) address.focus();
  else if (!notNull(email.value, "email_error")) email.focus();
  else if (!validateContactNumber(contact_number.value, "contact_number_error"))
    contact_number.focus();
  else if (!notNull(username.value, "username_error")) username.focus();
    ...
    document.getElementById("password_error").style.display = "block";
    document.getElementById("password_error").innerHTML =
      "must be of length 6 or more characterss!";
    password.focus();
    ...
    if (confirmation == "CONFIRM") {
      ...
      xhttp.open(
        "GET",
        "php/validateCredentials.php?action=store_admin_info&pharmacy_name=" +
          pharmacy_name.value +
          "&address=" +
          address.value +
          "&email=" +
...
function verifyEmailNumber() {
  ...
  xhttp.open(
    "GET",
    "php/validateCredentials.php?action=verify_email_number&email=" +
      email +
      "&contact_number=" +
      contact_number,
    false
  ...
  } else alert("Invalid email or contact number!");
}
...
function updateUsernamePassword() {
  ...
  if (!notNull(username.value, "username_error")) username.focus();
  else if (username.value.indexOf(" ") >= 0) {
    document.getElementById("username_error").style.display = "block";
    document.getElementById("username_error").innerHTML =
      "mustn't contain spaces!";
    ...
    xhttp.open(
      "GET",
      "php/validateCredentials.php?action=update_username_password&username=" +
        username.value +
        "&password=" +
        password.value +
        "&email=" +
